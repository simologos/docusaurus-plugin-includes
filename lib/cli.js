...
function copyFolders(cleanFirst, sharedFolders, siteDir) {
    ...
    if (!sharedFolders) {
        throw new Error(`${pluginLogPrefix}The configuration option 'sharedFolders' is not defined.`);
    }
    ...
    sharedFolders.forEach(folderEntry => {
        ...
        if (!fs_extra_1.default.pathExistsSync(sourceFolder)) {
            throw new Error(`${pluginLogPrefix}The configured source folder '${folderEntry.source}' doesn't exist`);
        }
    });
